...
import { normalizeCard, normalizeCardAsAgent } from '../lib/normalize-card.js';
import type { Category, HubCard, SortOption } from '../types.js';

/** Stub card name generated by onboarding fallback template — hidden in both tabs. */
const STUB_CARD_NAME = 'Custom (describe it)';

/** Discovery tab: Agents (one tile per agent) or Skills (one tile per skill). */
export type DiscoverTab = 'agents' | 'skills';
...
export function useCards(): UseCardsResult {
  ...
  const [rawUsesMap, setRawUsesMap] = useState<Record<string, number>>({});
  const [total, setTotal] = useState(0);
  const [activeTab, setActiveTab] = useState<DiscoverTab>('agents');
  ...
  const filteredCards = useMemo(() => allCards.filter((card) => {
    ...
    // capabilityType: substring match — agents tab checks all_capability_types, skills tab checks capability_types
    if (capabilityType.trim() !== '') {
      const needle = capabilityType.trim().toLowerCase();
      const haystack = activeTab === 'agents'
        ? (card.all_capability_types ?? card.capability_types ?? [])
        : (card.capability_types ?? []);
      if (!haystack.some((ct) => ct.toLowerCase().includes(needle))) return false;
    }
    return true;
  }), [allCards, category, minSuccessRate, verifiedOnly, capabilityType, activeTab]);
  ...
  const setActiveTabAndReset = useCallback((t: DiscoverTab) => { setActiveTab(t); setPage(1); }, []);
  ...
  return {
    ...
    error,
    activeTab,
    setActiveTab: setActiveTabAndReset,
    query,
