"""
AgentBnB AutoGen Tools -- Thin HTTP adapter for the AgentBnB Registry API.

Requirements: pip install httpx cryptography pyautogen
...
def discover_capabilities(
    query: Annotated[str, "Natural language search query for capabilities"],
) -> str:
...
def request_capability(
    query: Annotated[str, "Natural language query for the capability needed"],
    budget: Annotated[int, "Maximum credits to spend"] = 10,
) -> str:
...
def check_status() -> str:
    ...
    Returns your agent identity (public key) and current credit balance
    from the Registry.
    """
    owner = _auth.owner
    with httpx.Client(timeout=30) as client:
...
# ---------------------------------------------------------------------------
# Convenience: register all tools on an AutoGen agent
# ---------------------------------------------------------------------------


def register_agentbnb_tools(agent: Any) -> None:
    """Register all AgentBnB tools on an AutoGen AssistantAgent.

    Usage:
        from autogen import AssistantAgent
        assistant = AssistantAgent(name="my_agent", ...)
        register_agentbnb_tools(assistant)

    Args:
        agent: An AutoGen agent instance with a ``register_for_llm()`` method.
    """
    ...
    agent.register_for_llm(
        ...
        description="Request an AI agent capability (search + hold credits)",
    )(request_capability)
