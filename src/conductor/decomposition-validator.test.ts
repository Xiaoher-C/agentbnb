...
const defaultCtx = { max_credits: 100 };
...
describe('validateAndNormalizeSubtasks', () => {
  ...
  it('Test 8: Unknown role field is silently ignored (role validation removed)', () => {
    const raw = [{ id: 'step-1', description: 'do something', required_capability: 'text_gen', role: 'wizard' }];
    const result = validateAndNormalizeSubtasks(raw, defaultCtx);
    // role is no longer validated — unknown fields are ignored, not errors
  ...
  it('Test 10: estimated_credits > max_credits returns error', () => {
    ...
    const result = validateAndNormalizeSubtasks(raw, { max_credits: 100 });
  ...
  it('Test 12: Valid array with estimated_credits within bounds — normalizes correctly', () => {
    const raw = [
      {
        id: 'step-1',
        description: 'do something',
    ...
    expect(task.required_capability).toBe('text_gen');
  });
